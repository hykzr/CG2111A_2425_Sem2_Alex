...
def slamThread(setupBarrier:Barrier=None, readyBarrier:Barrier=None):
    ...
    subscribe(topic=SLAM_RESET_TOPIC, ensureReply=True, replyTimeout=1)
    ...
    try:
        while(not ctx.isExit()):
            ...
            message = messages[-1]
            if PubSubMsg.getTopic(message) == SLAM_RESET_TOPIC:
                print_orange("Resetting slam...")
                slam, _ = makeSlam()
                print_green("Reset slam done")
                continue
            # get the payload
        ...
        print_red(f"\rSLAM Thread Exception: {repr(e)}")
        traceback.print_exc()
        pass
    ...
    ctx.doExit()
    print_orange("\rExiting SLAM Thread")
    pass
...
def makeSlam():
    ...
    slamParamString = "\rSLAM Parameters:\n"
    slamParamString += f"\rNumber of Data Points per Round: {NUMBER_OF_DATAPOINTS_PER_ROUND}\n"
    slamParamString += f"\rScanning Frequency: {SCANNING_FREQUENCY}\n"
    slamParamString += f"\rScanning Field of View: {SCANNING_FIELD_OF_VIEW}\n"
    slamParamString += f"\rDistance if No Obstacle: {DISTANCE_IF_NO_OBSTACLE}\n"
    slamParamString += f"\rMinimum Samples: {min_samples}\n"
    slamParamString += f"\rMap Size (Pixels): {map_size_pixels}\n"
    slamParamString += f"\rMap Size (Meters): {map_size_meters}\n"
    slamParamString += f"\rHole Width (mm): {hole_width_mm}\n"
    slamParamString += f"\rMap Quality: {MAP_QUALITY}\n"
    slamParamString += f"\rLidar Offset (Degrees): {LIDAR_OFFSET_DEGREES}\n"
