...
ROBOT_WIDTH_METERS = 0.25 
ROBOT_HEIGHT_METERS = 0.35
...
def lidarDisplayProcess(setupBarrier:Barrier=None, readyBarrier:Barrier=None):
    ...
    # Printing Parameters
    paramsStr = f"\rDisplay Parameters:\n"
    paramsStr += f"\rLidar Offset (Degrees): {LIDAR_OFFSET_DEGREES}\n"
    paramsStr += f"\rMap Size (Pixels): {MAP_SIZE_PIXELS}\n"
    paramsStr += f"\rMap Size (Meters): {MAP_SIZE_METERS}\n"
    paramsStr += f"\rRobot Width (Meters): {ROBOT_WIDTH_METERS}\n"
    paramsStr += f"\rRobot Height (Meters): {ROBOT_HEIGHT_METERS}\n"
    paramsStr += f"\rMax Framerate: {FRAMERATE}\n"
    ...
    try:
        ...
        print_red(f"\rLidar Display Thread Exception: {repr(e)}")
        traceback.print_exc()
        pass
    plt.close()
    # Shutdown and exit the thread gracefully
    ctx.doExit()
    print_orange("\rExiting Lidar Display Process")
    pass
