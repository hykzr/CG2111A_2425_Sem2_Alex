...
##################################
arduino_port_setting = None
arduino_starting = False
def restartSerial():
    global arduino_port_setting, arduino_starting
    arduino_starting = True
    closeSerial()
    res = startSerial(**arduino_port_setting)
    time.sleep(1)
    arduino_starting = False
    return res
def startSerial(
    ...
    global arduino_port_setting
    # save the current serial port setting, 
    # so that we can use the same setting to restart Arduino
    arduino_port_setting={
        "portName": portName,
        "baudRate": baudRate,
        "byteSize": byteSize,
        "parity": parity,
        ...
        "failedAttemptWaitSec": failedAttemptWaitSec
    }
    """
...
def readSerial(bytes_to_read:int):
    ...
    """
    global _MYSERIAL, arduino_starting
    if arduino_starting:
        print("waiting for arduino to start")
        while arduino_starting:
            time.sleep(0.01)
    if (_MYSERIAL == None):
