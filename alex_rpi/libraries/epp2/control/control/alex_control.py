...
def receivePacket(exitFlag:Event=_EXIT_EVENT):
    ...
    while(not exitFlag.is_set()):
        #Add Try Except to handle errors, this is crucial for safely restarting Arduino
        try:
            res_size, res = readSerial(target_packet_size-buffer_size)
        except Exception as e:
            print(f"Error While Receiving Packet: {repr(e)}")
            continue
...
def sendPacket(packetType:TPacketType, commandType:TCommandType, params:list, exitFlag:Event=None):
    global arduino_responded
    """
    ...
    Args:
        ...
        exitFlag: the exiting event
    Returns:
        None
    """
    # Added: poll until arduino responded to prevent crashing Arduino
    if not arduino_responded:
        print("Arduino not responded yet, waiting for it to respond")
        while not arduino_responded:
            time.sleep(0.01)
            #will exit gracefully even while waiting Arduino response
            if exitFlag and exitFlag.is_set():
                return
    arduino_responded = False
...
def parseParams(p, num_p, inputMessage, acceptInput = True):
    """
    ...
    Args:
        ...
        acceptInput: whether letting user input addtional params, should be False when calling from remote side
    ...
    elif (len(p) < num_p) and (inputMessage != None) and acceptInput:
...
def parseUserInput(input_str:str, exitFlag:Event = _EXIT_EVENT, acceptInput = True):
    """
    ...
    if command == "f":
        ...
        return (packetType, commandType,  params, acceptInput) if params != None else print("Invalid Parameters")
    elif command== "b":
